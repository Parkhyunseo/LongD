...
public class Sequences : MonoBehaviour
{
	...
	void Start()
	{
        Vector3[] pathies = new Vector3[path.Length];

        for (int i = 0; i < path.Length; i++)
            pathies[i] = path[i].position;
		...
		mySequence.SetLoops(20, LoopType.Yoyo);
        */
        target.transform.DOPath(pathies, 5, PathType.Linear, PathMode.TopDown2D);
